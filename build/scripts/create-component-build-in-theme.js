...
allStyleFilePath.forEach((scssFile, index) => {
    ...
    `;
    const darkRes = darkImport + fileContent.replace("host", "host[theme='dark']");
    const lightRes = lightImport + fileContent.replace("host", "host[theme='light']");
    ...
    angularString += `
    {
        ...
        "bundleName": "themes/components/${fileName}-dark.scss",
        "inject": false
    },
    {
        "input": "src/jigsaw/common/core/theming/prebuilt/build-in-theme/${fileName}-light.scss",
        "bundleName": "themes/components/${fileName}-light.scss",
        "inject": false
    },`;
});
