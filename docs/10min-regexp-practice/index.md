...
不卖关子了，直接上答案，看这个表达式 `[\s\S]` 能够匹配的是啥？字面上看，它能够匹配一个_空格或者非空格_，换句话说，用它就能够匹配任意字符了。任何一个字符要不就是空格，要不就不是空格，不是吗？所以，我们也是分分钟就把正则写好了：
...
> 有人会问，这样的 `[\b\B]` 正则表达式不也能够匹配任意字符吗？
> 是的没错，你很聪明。`[\b\B]` 和 `[\w\W]` 等都能够达到类似效果。他们的原理都是一样的。
