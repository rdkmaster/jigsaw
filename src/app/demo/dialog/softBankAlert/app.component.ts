...
export class DialogMiscDemoComponent {
    ...
    * */
    popup() {
        const popupInfo = this._popupService.popup(SoftBankAlertInfoComponent, this._getModalOptions());
     ...
     * */
    popupAtPoint(event) {
        const popupInfo = this._popupService.popup(SoftBankAlertWarningComponent, this._getUnModalOptions(event));
        if(popupInfo.popupRef instanceof ComponentRef){
            popupInfo.popupRef.instance.answer.subscribe(answer => {
                this.disposeAnswer(answer, popupInfo)
            })
