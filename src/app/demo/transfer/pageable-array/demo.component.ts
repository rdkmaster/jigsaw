...
export class TransferPageableArrayComponent {
    constructor(private _http: HttpClient) {
        this.data = new PageableArray(_http, 'http://localhost:5812/rdk/service/app/example/server/my_service');
        this.data.fromAjax();

        this.selectedCountries = new ArrayCollection();
        this.selectedCountries.http = _http;
        this.selectedCountries.fromAjax('http://localhost:5812/rdk/service/app/example/server/my_service');
