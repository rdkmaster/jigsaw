...
export class FormDisplayRendererDataTestDemoComponent {
    ...
    public formio = {
        ...
        "data": this.getRenderAsData(this.formDisplayRows)
    }

    public getRenderAsData(rowCount: number): any[] {
        const rows = [];
        rows.push([
            {"value": `<div style="background: grey; width: fit-content; border-radius: 5px; padding: 0 8px 0 8px;">目标名称</div>`,
                "isHeader": true, "renderAs": "html"},
        ...
        const firstRow = [
            {"value": "行1数据1", "renderAs": "tag"},
            {"value": "行1数据2", "renderAs": "tag"},
            {"value": "行1数据3", "renderAs": "tag"},
            {"value": "行1数据4", "renderAs": "tag"},
            {"value": "行1数据5", "renderAs": "tag"},
            {"value": "行1数据6", "renderAs": "tag"},
            {"value": "行1数据7", "renderAs": "tag"},
            {"value": "行1数据8", "renderAs": "tag"},
            {"value": "行1数据9", "renderAs": "tag"},
            {"value": "行1数据10", "renderAs": "tag"}
        ];
        for (let i = 0; i < rowCount; i++) {
            const newRow = firstRow.map(item => ({"value": `行${i + 1}${item.value.slice(2)}`, "renderAs": "tag"}));
            rows.push(newRow);
        }
        return rows;
    }
