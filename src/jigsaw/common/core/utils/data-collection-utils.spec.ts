...
describe('Unit Test for data-collection-utils/getColumn()', () => {
    it('getColumn - normal', (done) => {
        ...
        expect(JSON.stringify(col)).toEqual(JSON.stringify(['12', 22, '32']));
        done();
    });
...
describe('Unit Test for data-collection-utils/getRow()', () => {
    it('getRow - normal', (done) => {
        ...
        expect(JSON.stringify(row)).toEqual(JSON.stringify([21, '22', 23, '24']));
        expect(row === m[1]).toEqual(false);
        done();
    });
...
describe('Unit Test for data-collection-utils/distinct()', () => {
    it('distinct - normal1', (done) => {
        ...
        expect(JSON.stringify(list)).toEqual(JSON.stringify([11, 12, 13, 14]));
        expect(list === r).toEqual(false);
        done();
    });
    it('distinct - normal2', (done) => {
        const list = [11, 11, 13, 13];
        const r = distinct(list);
        expect(JSON.stringify(r)).toEqual(JSON.stringify([11, 13]));
        expect(list === r).toEqual(false);
        done();
    });
...
describe('Unit Test for data-collection-utils/group()', () => {
    it('group - normal', (done) => {
        ...
        const g = group(m, 0);
        expect(JSON.stringify(g._$groupItems)).toEqual(JSON.stringify(["一", "二", "三", "四", "五", "六", "日"]));
        let sub = g['一'];
        let test = [['一', '南京', '20', '10'], ['一', '上海', '22', '12'], ['一', '深圳', '30', '23']];
        expect(JSON.stringify(sub)).toEqual(JSON.stringify(test));
        sub = g['二'];
    ...
    it('group - invalid group field', (done) => {
        ...
        expect(group(m, 10)).toEqual(undefined);
        expect(group(m, -10)).toEqual(undefined);
        done();
    });
});
...
describe('Unit Test for data-collection-utils/flat()', () => {
    it('flat - normal', (done) => {
        ...
        expect(JSON.stringify(f)).toEqual(JSON.stringify(test));
        done();
    });
});
...
describe('Unit Test for data-collection-utils/aggregate()', () => {
    it('aggregate - normal', (done) => {
        ...
        expect(JSON.stringify(r)).toEqual(JSON.stringify(['aa', '南京', 141, 14]));

        r = aggregate(m, [{algorithm: 'max', index: 2}, {algorithm: 'min', index: 3}]);
        expect(JSON.stringify(r)).toEqual(JSON.stringify(['aa', '南京', 30, 9]));

        r = aggregate(m, [{algorithm: 'head', index: 2}, {algorithm: 'tail', index: 3}]);
        expect(JSON.stringify(r)).toEqual(JSON.stringify(['aa', '南京', '21', 12]));
        ...
        expect(JSON.stringify(r)).toEqual(JSON.stringify(['aa', '南京', '14.10', 12]));

        expect(() => r = aggregate(m, [{algorithm: null, index: 2}]))
            .toThrowError('unsupported aggregate algorithm: null');
        done();
    });
    it('aggregate - invalid matrix', (done) => {
        let r = aggregate(null, [{algorithm: 'sum', index: 2}, {algorithm: 'average', index: 3}]);
        expect(JSON.stringify(r)).toEqual(JSON.stringify([]));
        done();
    });
    it('aggregate - invalid matrix', (done) => {
