...
import {JigsawMobileRadioModule} from "./radio";
...
@Component({
    selector: 'jigsaw-mobile-radios-lite, jm-radios-lite',
    template: `
        <jm-radios [(value)]="value" (valueChange)="radioChange($event)" [trackItemBy]="trackItemBy">
            <jm-radio-option *ngFor="let item of data; trackBy: _$trackByFn" [value]="item" [disabled]="item?.disabled">
    ...
    providers: [
        {provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => JigsawMobileRadiosLite), multi: true},
    ]
})
export class JigsawMobileRadiosLite extends AbstractJigsawComponent implements ControlValueAccessor {
...
@NgModule({
    imports: [CommonModule, JigsawMobileRadioModule],
    declarations: [JigsawMobileRadiosLite],
    exports: [JigsawMobileRadiosLite]
})
export class JigsawMobileRadioLiteModule {
}
