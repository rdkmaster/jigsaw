...
export abstract class JigsawSelectBase extends AbstractJigsawComponent implements IJigsawFormControl, ControlValueAccessor, OnDestroy {
    ...
    private _validDataAllNotSelected(): boolean {
        ...
        return this.searchable && this._$selectedItems.every(item => !this._getValidData().find(data => CommonUtils.compareWithKeyProperty(item, data, <string[]>this.trackItemBy)))
    }
    ...
    protected _validDataAllSelected(): boolean {
        ...
        return this.searchable && this._getValidData().every(data => !!this._$selectedItems.find(item => CommonUtils.compareWithKeyProperty(item, data, <string[]>this.trackItemBy)))
    }
