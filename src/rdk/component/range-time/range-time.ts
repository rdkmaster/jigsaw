...
export class RdkRangeTime extends AbstractRDKComponent implements OnInit {
    ...
    private _calculateLimitEnd(): WeekTime {
        ...
        if (item && item.span) {
            let calculateTime: WeekTime = RdkRangeTime._calculateLimitEnd(TimeService.convertValue(this._$beginDate, <TimeGr>this._timeStart.gr), item.span, <TimeGr>this._timeStart.gr);
